...
const STATUS_STYLES: Record<string, string> = {
  new: 'bg-brand-ash/30 text-brand-silver border border-brand-ash/40',
  ...
  expired: 'bg-brand-ash/20 text-brand-ash border border-brand-ash/30',
};
    ...
    <div className="space-y-4">
      {leads.length === 0 ? (
        <div className="rounded-xl border border-brand-graphite bg-brand-jet-light px-6 py-16 text-center">
          <Eye className="mx-auto text-brand-ash" size={36} />
          ...
          {leads.map((lead) => (
            ...
            >
              ...
              {lead.photos && lead.photos.length > 0 && (
                <div className="mt-4">
                  <p className="text-caption font-semibold uppercase tracking-wider text-brand-ash mb-2">
              ...
              <div className="mt-4 rounded-lg border border-brand-ash bg-brand-graphite/30 p-4">
                <div className="grid grid-cols-3 gap-4 text-center">
                  ...
                  <div>
                    <p className="text-caption text-brand-ash">GST (5%)</p>
                    <p className="font-mono font-bold text-brand-white">
                      ${lead.estimatedGst?.toFixed(2) || '0.00'}
                    </p>
                  </div>
                  <div>
                    <p className="text-caption text-brand-ash">Total</p>
                    <p className="font-mono font-bold text-brand-white">
                      ${lead.estimatedTotal?.toFixed(2) || '0.00'}
                    </p>
                  </div>
              ...
              <div className="mt-4 flex flex-wrap items-center gap-2">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-lg border border-brand-graphite bg-brand-red/20 hover:bg-brand-red/30 px-3 py-2 text-caption text-brand-red hover:text-brand-red transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isCreatingBooking ? (
                    <>
